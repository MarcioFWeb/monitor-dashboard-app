...
import { CHART_URL_MEMORY, chartDataDecorator } from '../../ValueObjects/Helpers/chartDataHelper';
...
export default function ChartCPUUsage() {
  ...
  useEffect(() => {
    const fetchData = async () =>{
      setLoading(true);
      try {
        const {data: response} = await axios.get(CHART_URL_MEMORY);
  ...
  return (
    ...
    {!loading && (
      <>
        <ChartDefaultContent title={'Memory Usage'} chartData={chartData}/>        
      </>
    )}
    </>
  );  
